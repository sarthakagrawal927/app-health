...
in memory and SHALL NOT place it in browser storage, URLs, logs, or analytics.
...
The SDK ingest route SHALL remain reachable without an owner dashboard unlock
...
apply the same owner-secret validation to every owner API request received there.
...
The production release SHALL use the account's existing Workers subscription
and included D1 and Analytics Engine allowances and SHALL NOT activate Zero
Trust or another paid add-on.
